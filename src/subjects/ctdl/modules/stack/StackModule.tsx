...
import { runStack } from "../../engine/stack";
import { STACK_EXAMPLES, STACK_CODE } from "../../data/stackExamples";
...
export default function StackModule() {
  const [selectedId, setSelectedId] = useState(STACK_EXAMPLES[0].id);
  const example = STACK_EXAMPLES.find((e) => e.id === selectedId)!;
  const { steps, summary, opLog } = useMemo(() => runStack(example.spec), [example]);
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div>
        <h2 className="text-lg font-semibold mb-2">4. Stack (ngăn xếp)</h2>
        <p className="text-sm text-slate-400">
          {example.label}. Từng dòng lệnh của push / pop / count trên danh sách liên kết, đúng code của thầy. <span className="text-exam-accent">Xanh dương</span> = node đang xử lý ·{" "}
          <span className="text-exam-good">xanh lá</span> = vừa nối xong · <span className="text-exam-bad">đỏ</span> = sắp bị delete.
        </p>
      </div>
      <ExamplePicker
        options={STACK_EXAMPLES.map((e) => ({ id: e.id, label: e.label }))}
      ...
      <CodeBlock title="Code chuẩn (viết hàm trên đề)" code={STACK_CODE} />
    </div>
  );
}
